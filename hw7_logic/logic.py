...
def rstr(x):
    if isinstance(x, tuple):
        return str(tuple(map(rstr, x)))
    if isinstance(x, list):
        return str(list(map(rstr, x)))
    if isinstance(x, set):
        return str(set(map(rstr, x)))
...
class Expression:
    ...
    def ensureType(self, arg, wantedType):
        if not isinstance(arg, wantedType):
            raise Exception(
                "%s: wanted %s, but got %s" % (self.__class__.__name__, wantedType, arg)
            )
        return arg
    ...
    def __repr__(self):
        if not self.strRepn:
            self.strRepn = self.computeStrRepn()
        return self.strRepn
...
class Variable(Term):
    def __init__(self, name):
        if not name.startswith("$"):
            raise Exception('Variable must start with "$", but got %s' % name)
        self.name = name
        self.strRepn = None
...
class Constant(Term):
    def __init__(self, name):
        if not name[0].islower():
            raise Exception(
                "Constants must start with a lowercase letter, but got %s" % name
            )
        self.name = name
        self.strRepn = None
...
class Atom(Formula):
    def __init__(self, name, *args):
        if not name[0].isupper():
            raise Exception(
                "Predicates must start with a uppercase letter, but got %s" % name
            )
        self.name = name
...
class And(Formula):
    ...
    def computeStrRepn(self):
        return "And(" + str(self.arg1) + "," + str(self.arg2) + ")"
...
class Or(Formula):
    ...
    def computeStrRepn(self):
        return "Or(" + str(self.arg1) + "," + str(self.arg2) + ")"
...
class Implies(Formula):
    ...
    def computeStrRepn(self):
        return "Implies(" + str(self.arg1) + "," + str(self.arg2) + ")"


# Example: Exists($x,Lives(john, $x))
class Exists(Formula):
    def __init__(self, var, body):
        self.var = self.ensureType(to_expr(var), Variable)
...
def flatten_and(form):
    if form.isa(And):
        return flatten_and(form.arg1) + flatten_and(form.arg2)
    else:
        return [form]
...
def flatten_or(form):
    if form.isa(Or):
        return flatten_or(form.arg1) + flatten_or(form.arg2)
    else:
        return [form]
...
def AntiReflexive(predicate):
    ...
    return Forall(
        "$x",
        Forall("$y", Implies(Atom(predicate, "$x", "$y"), Not(Equals("$x", "$y")))),
    )
...
def unify(form1, form2, subst):
    if form1.isa(Variable):
        return unify_terms(form1, form2, subst)
    if form1.isa(Constant):
        return unify_terms(form1, form2, subst)
    if form1.isa(Atom):
        return (
            form2.isa(Atom)
            and form1.name == form2.name
            and len(form1.args) == len(form2.args)
            and all(
                unify(form1.args[i], form2.args[i], subst)
                for i in range(len(form1.args))
            )
        )
    if form1.isa(Not):
    ...
    if form1.isa(And):
        return (
            form2.isa(And)
            and unify(form1.arg1, form2.arg1, subst)
            and unify(form1.arg2, form2.arg2, subst)
        )
    if form1.isa(Or):
        return (
            form2.isa(Or)
...
def unify_terms(a, b, subst):
    # print 'unify_terms', a, b, rstr(subst)
    a = get_subst(subst, a)
    b = get_subst(subst, b)
    if a == b:
        return True
...
def apply_subst(form, subst):
    ...
    if form.isa(Variable):
        # print 'apply_subst', rstr(form), rstr(subst), rstr(subst.get(form, form))
        # return subst.get(form, form)
        return get_subst(subst, form)
    if form.isa(Constant):
        return form
    if form.isa(Atom):
        return Atom(*[form.name] + [apply_subst(arg, subst) for arg in form.args])
    if form.isa(Not):
        return Not(apply_subst(form.arg, subst))
    if form.isa(And):
        return And(apply_subst(form.arg1, subst), apply_subst(form.arg2, subst))
    if form.isa(Or):
        return Or(apply_subst(form.arg1, subst), apply_subst(form.arg2, subst))
...
def without_element_at(items, i):
    return items[0:i] + items[i + 1 :]


def negate_formula(item):
...
# - Remove duplicates
# - Sort the list
def reduce_formulas(items, mode):
    for i in range(len(items)):
        for j in range(i + 1, len(items)):
            if negate_formula(items[i]) == items[j]:
                if mode == And:
                    return [AtomFalse]
                elif mode == Or:
                    return [AtomTrue]
...
def all_free_vars(form):
    ...
    def recurse(form, boundVars):
        if form.isa(Variable):
            if form not in boundVars:
                variables.append(form)
        elif form.isa(Constant):
            pass
        elif form.isa(Atom):
            for arg in form.args:
                recurse(arg, boundVars)
        elif form.isa(Not):
            recurse(form.arg, boundVars)
        elif form.isa(And):
            recurse(form.arg1, boundVars)
            recurse(form.arg2, boundVars)
        elif form.isa(Or):
            recurse(form.arg1, boundVars)
            recurse(form.arg2, boundVars)
        elif form.isa(Implies):
...
def substitute_free_vars(form, var, obj):
    def recurse(form, boundVars):
        if form.isa(Variable):
            ...
            if form.var == var:
                return form  # Don't substitute inside
    ...
    def applyRule(self, form):
        ...
        def removeImplications(form):
            if form.isa(Atom):
                return form
            if form.isa(Not):
                return Not(removeImplications(form.arg))
            if form.isa(And):
                return And(removeImplications(form.arg1), removeImplications(form.arg2))
            if form.isa(Or):
                return Or(removeImplications(form.arg1), removeImplications(form.arg2))
            if form.isa(Implies):
                return Or(
                    Not(removeImplications(form.arg1)), removeImplications(form.arg2)
                )
            if form.isa(Exists):
                return Exists(form.var, removeImplications(form.body))
            if form.isa(Forall):
                return Forall(form.var, removeImplications(form.body))
            raise Exception("Unhandled: %s" % form)
        ...
        def pushNegationInwards(form):
            ...
            if form.isa(Not):
                ...
                if form.arg.isa(And):  # De Morgan
                    return Or(
                        pushNegationInwards(Not(form.arg.arg1)),
                        pushNegationInwards(Not(form.arg.arg2)),
                    )
            ...
            if form.isa(And):
                return And(
                    pushNegationInwards(form.arg1), pushNegationInwards(form.arg2)
                )
            if form.isa(Or):
                return Or(
                    pushNegationInwards(form.arg1), pushNegationInwards(form.arg2)
            ...
            if form.isa(Implies):
                return Or(
                    Not(pushNegationInwards(form.arg1)), pushNegationInwards(form.arg2)
                )
            if form.isa(Exists):
                return Exists(form.var, pushNegationInwards(form.body))
            if form.isa(Forall):
                return Forall(form.var, pushNegationInwards(form.body))
            raise Exception("Unhandled: %s" % form)
        ...
        def standardizeVariables(form, subst):
            if form.isa(Variable):
                if form not in subst:
                    raise Exception("Free variable found: %s" % form)
                return subst[form]
            if form.isa(Constant):
                return form
            if form.isa(Atom):
                return Atom(
                    *(
                        [form.name]
                        + [standardizeVariables(arg, subst) for arg in form.args]
                    )
                )
            if form.isa(Not):
                return Not(standardizeVariables(form.arg, subst))
            if form.isa(And):
                return And(
                    standardizeVariables(form.arg1, subst),
                    standardizeVariables(form.arg2, subst),
                )
            if form.isa(Or):
                return Or(
                    standardizeVariables(form.arg1, subst),
            ...
            if form.isa(Exists):
                ...
                return Exists(
                    newSubst[form.var], standardizeVariables(form.body, newSubst)
                )
            if form.isa(Forall):
                newSubst = updateSubst(subst, form.var)
                return Forall(
        ...
        def skolemize(form, subst, scope):
            ...
            if form.isa(Atom):
                return Atom(
                    *[form.name] + [skolemize(arg, subst, scope) for arg in form.args]
                )
            if form.isa(Not):
                return Not(skolemize(form.arg, subst, scope))
            if form.isa(And):
                return And(
                    skolemize(form.arg1, subst, scope),
                    skolemize(form.arg2, subst, scope),
                )
            if form.isa(Or):
                return Or(
                    skolemize(form.arg1, subst, scope),
            ...
            if form.isa(Exists):
                ...
                if len(scope) == 0:
                    ...
                    return Forall(
                        form.var, Or(Not(skolem), skolemize(form.body, subst, scope))
                    )
            if form.isa(Forall):
        ...
        def removeUniversalQuantifiers(form):
            if form.isa(Atom):
                return form
            if form.isa(Not):
                return Not(removeUniversalQuantifiers(form.arg))
            if form.isa(And):
                return And(
                    removeUniversalQuantifiers(form.arg1),
                    removeUniversalQuantifiers(form.arg2),
                )
            if form.isa(Or):
                return Or(
                    removeUniversalQuantifiers(form.arg1),
            ...
            if form.isa(Forall):
                return removeUniversalQuantifiers(form.body)
            raise Exception("Unhandled: %s" % form)
        ...
        def distribute(form):
            if form.isa(Atom):
                return form
            if form.isa(Not):
                return Not(distribute(form.arg))
            if form.isa(And):
                return And(distribute(form.arg1), distribute(form.arg2))
            if form.isa(Or):
                ...
                return Or(f1, f2)
            if form.isa(Exists):
                return Exists(form.var, distribute(form.body))
            if form.isa(Forall):
                return Forall(form.var, distribute(form.body))
            raise Exception("Unhandled: %s" % form)
        ...
        conjuncts = [
            OrList(reduce_formulas(flatten_or(f), Or)) for f in flatten_and(newForm)
        ]
        # print rstr(form), rstr(conjuncts)
        assert len(conjuncts) > 0
        if any(x == AtomFalse for x in conjuncts):
            return [AtomFalse]
        if all(x == AtomTrue for x in conjuncts):
            return [AtomTrue]
        conjuncts = [x for x in conjuncts if x != AtomTrue]
        results = reduce_formulas(conjuncts, And)
        if len(results) == 0:
            results = [AtomFalse]
    ...
    def applyRule(self, form1, form2):
        ...
        for i, item1 in enumerate(items1):
            for j, item2 in enumerate(items2):
                subst = {}
                if unify(negate_formula(item1), item2, subst):
                    newItems1 = without_element_at(items1, i)
                    newItems2 = without_element_at(items2, j)
                    newItems = [
                        apply_subst(item, subst) for item in newItems1 + newItems2
                    ]
                    ...
                    result = OrList(reduce_formulas(newItems, Or))
...
def perform_model_checking(allForms, findAll, objects=None, verbose=0):
    if verbose >= 3:
        print(("perform_model_checking", rstr(allForms)))
    ...
    # allForms = reduce_formulas(allForms, And)
    allForms = [universal_interpret(form) for form in allForms]
    allForms = list(set(allForms) - set([AtomTrue, AtomFalse]))
    if verbose >= 3:
        print(("All Forms:", rstr(allForms)))

    if allForms == []:
        return [set()]  # One model
    if allForms == [AtomFalse]:
        return []  # No models
    ...
    atomForms = [
        (atom, [form for form in allForms if atom in all_subexpressions(form)])
        for atom in atoms
    ]
    # Degree heuristic
...
def propositionalize(forms, objects=None):
    ...
    def convert(form, subst):
        ...
        if form.isa(Not):
            return Not(convert(form.arg, subst))
        if form.isa(And):
            return And(convert(form.arg1, subst), convert(form.arg2, subst))
        if form.isa(Or):
            return Or(convert(form.arg1, subst), convert(form.arg2, subst))
        if form.isa(Implies):
            return Implies(convert(form.arg1, subst), convert(form.arg2, subst))
        if form.isa(Exists):
            return OrList(
                [
                    convert(form.body, dict(list(subst.items()) + [(form.var, obj)]))
                    for obj in objects
                ]
            )
        if form.isa(Forall):
...
def universal_interpret_atom(atom):
    if atom.name == "Equals":
...
def universal_interpret(form):
    ...
    if form.isa(Constant):
        return form
    if form.isa(Atom):
        result = universal_interpret_atom(form)
        if result != None:
            return result
...
def interpret_form(form, model):
    if form.isa(Atom):
        return form in model
    if form.isa(Not):
        return not interpret_form(form.arg, model)
    if form.isa(And):
        return interpret_form(form.arg1, model) and interpret_form(form.arg2, model)
    if form.isa(Or):
        return interpret_form(form.arg1, model) or interpret_form(form.arg2, model)
    if form.isa(Implies):
        return not interpret_form(form.arg1, model) or interpret_form(form.arg2, model)
    raise Exception("Unhandled: %s" % form)
...
class Derivation:
    def __init__(self, form, children, cost, derived):
        ...
        self.derived = (
            derived  # Whether this was derived (as opposed to added by the user).
        )

    def __repr__(self):
        return "Derivation(%s, cost=%s, permanent=%s, derived=%s)" % (
            self.form,
            self.cost,
            self.permanent,
            self.derived,
...
def show_kb_response(response, verbose=1):
    if isinstance(response, KBResponse):
        ...
        print(
            ("Yes: %s" % rstr([obj for obj, status in items if status == ENTAILMENT]))
        )
        print(
            ("Maybe: %s" % rstr([obj for obj, status in items if status == CONTINGENT]))
        )
        print(
            ("No: %s" % rstr([obj for obj, status in items if status == CONTRADICTION]))
        )
...
class KnowledgeBase:
    ...
    def dump(self):
        ...
        for deriv in list(self.derivations.values()):
            print(
                (
                    ("-" if deriv.derived else "*"),
                    deriv if self.verbose >= 2 else deriv.form,
                )
            )
    ...
    def query(self, form, modify):
        ...
        if len(freeVars) > 0:
            if modify:
                raise Exception(
                    "Can't modify database with a query with free variables: %s" % form
                )
            var = freeVars[0]
            allForms = AndList(
                [deriv.form for deriv in list(self.derivations.values())]
            )
            if allForms == AtomTrue:
                return {}  # Weird corner case
            objects = all_constants(allForms)
            # Try binding |var| to |obj|
            response = {}
            for obj in objects:
                response[(var, obj)] = self.query(
                    substitute_free_vars(form, var, obj), modify
                )
            return response
    ...
    def addDerivation(self, deriv):
        ...
        if oldDeriv == None and deriv.cost <= maxCost:
            ...
            if self.verbose >= 3:
                print(("add %s [%s derivations]" % (deriv, len(self.derivations))))

            if self.modelChecking:
                allForms = [deriv.form for deriv in list(self.derivations.values())]
                models = perform_model_checking(
                    allForms, findAll=False, verbose=self.verbose
                )
                if len(models) == 0:
                    return False
                else:
            ...
            # Apply rules forward
            if not self.applyUnaryRules(deriv):
                return False
    ...
    def ensureFormulas(self, rule, formulas):
        if isinstance(formulas, list) and all(
            formula == False or isinstance(formula, Formula) for formula in formulas
        ):
            return formulas
        raise Exception(
            "Expected list of Formulas, but %s returned %s" % (rule, formulas)
        )
        ...
        for rule in self.rules:
            ...
            for newForm in self.ensureFormulas(rule, rule.applyRule(deriv.form)):
                if not self.addDerivation(
                    Derivation(
                        newForm, children=[deriv], cost=deriv.cost + 1, derived=True
                    )
                ):
                    return False
        return True
    ...
    def applyBinaryRules(self, deriv1, deriv2):
        for rule in self.rules:
            ...
            for newForm in self.ensureFormulas(
                rule, rule.applyRule(deriv1.form, deriv2.form)
            ):
                if not self.addDerivation(
                    Derivation(
                        newForm,
                        children=[deriv1, deriv2],
                        cost=deriv1.cost + deriv2.cost + 1,
                        derived=True,
                    )
                ):
                    return False
...
def create_resolution_kb():
    return KnowledgeBase(
        standardizationRule=ToCNFRule(), rules=[ResolutionRule()], modelChecking=False
    )


def create_model_checking_kb():
