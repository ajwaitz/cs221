...
class NLTKProcessor(NaturalLanguageProcessor):
    ...
    def process_with_nltk(self):
        ...
        for i in range(len(self.pos_tags)):
            if self.tokens[i] == "Does":
                self.pos_tags[i] = "VBZ"  # Mistagged as NNP
            if self.tokens[i] == "Did":
                self.pos_tags[i] = "VBZ"  # Mistagged as NNP
            if self.tokens[i] == "red":
                self.pos_tags[i] = "JJ"  # Mistagged as VBN
            if self.tokens[i] == "lives":
                self.pos_tags[i] = "VBZ"  # Mistagged as NNS
            if self.tokens[i] == "likes":
                self.pos_tags[i] = "VBZ"  # Mistagged as IN
            if self.tokens[i] == "like":
                self.pos_tags[i] = "VB"  # Mistagged as IN
        ...
        self.tokens = [
            lemmatize(token, tag) for token, tag in zip(self.tokens, self.pos_tags)
        ]
...
def get_category_processor(categories):
    ...
    for cat in categories:
        for word in categories[cat]:
            assert word not in wordToCategory, "%s is in both %s and %s" % (
                word,
                wordToCategory[word],
                cat,
            )
            capital = word.capitalize()
            assert capital not in wordToCategory, "%s is in both %s and %s" % (
                capital,
                wordToCategory[capital],
                cat,
            )
...
def lambda_rstr(x):
    if isinstance(x, tuple):
        return str(tuple(map(lambda_rstr, x)))
    if callable(x):
        v = Constant("v")
        try:
            ...
            try:
                return "(%s => %s)" % (
                    v,
                    lambda_rstr(x(lambda a: Constant("v(" + str(a) + ")"))),
                )
            except:
                pass
    return rstr(x)
...
def create_base_language_processor():
    ...
    return get_category_processor(categories)


def create_base_english_grammar():
    ...
    rules.append(
        GrammarRule(
            ...
            lambda args: Forall(
                "$x", Implies(Atom(args[0].title(), "$x"), Atom(args[1].title(), "$x"))
            ),
        )
    )
    ...
    rules.append(
        GrammarRule("$Question", ["is", "$Clause-be", "?"], lambda args: args[0])
    )
    rules.append(
        GrammarRule(
            "$Clause-be",
    ...
    rules.append(
        GrammarRule(
            ...
            lambda args: Exists(
                ...
                And(
                    Atom(args[2].title(), "$y"),
                    Atom(args[1].title(), args[0].lower(), "$y"),
                ),
            ),
        )
    )
...
class ParserDerivation:
    ...
    def dump(self, indent=""):
        print(
            "%s%s: score=%s, rule: %s"
            % (indent, lambda_rstr(self.form), self.score, self.rule)
        )
...
def parse_utterance(utterance, rules, verbose=0):
    def isCat(x):
        return x.startswith("$")
    ...
    def applyRule(start, end, mid, rule, rhsIndex, children, score):
        ...
        if rhsIndex == len(rule.rhs):
            deriv = ParserDerivation(
                rule.sem([child.form for child in children]),
                rule,
                children,
                score + rule.score,
            )
            if verbose >= 3:
                print(
                    "applyRule: %s:%s%s %s += %s"
                    % (start, end, tokens[start:end], rule.lhs, rstr(deriv.form))
                )
    ...
    chart = [
        None
    ] * n  # start => end => category => top K derivations (logical form, score)
    ...
    for start in range(0, n):
        ...
        if utterance.tokens[start] in ("be", "not"):
            continue
        chart[start][start + 1]["$" + utterance.pos_tags[start]].append(
            ParserDerivation(utterance.tokens[start], None, [], 0)
        )
    ...
    for length in range(1, n + 1):  # For each span length...
        for start in range(n - length + 1):  # For each starting position
            ...
            for cat in list(cell.keys()):
                cell[cat] = sorted(cell[cat], key=lambda deriv: -deriv.score)[
                    0:beamSize
                ]
...
def train_grammar(rules):
    ...
    examples.append(
        ("John was born in Seattle.", ("tell", Atom("Bear_in", "john", "seattle")))
    )
    examples.append(
        ("John lives in Seattle.", ("tell", Atom("Live_in", "john", "seattle")))
    )
    examples.append(
        ("John lives in New York.", ("tell", Atom("Live_in", "john", "new_york")))
    )
    examples.append(
        (
            "John does not live in New York.",
            ("tell", Not(Atom("Live_in", "john", "new_york"))),
        )
    )
    ...
    examples.append(
        (
            ...
            ("tell", And(Atom("Big", "new_york"), Atom("City", "new_york"))),
        )
    )
    examples.append(
        ("If it is raining, it is wet.", ("tell", Implies(Atom("Rain"), Atom("Wet"))))
    )

    # Coordination
    examples.append(
        (
            "John and Bill are cats.",
            ("tell", And(Atom("Cat", "john"), Atom("Cat", "bill"))),
        )
    )
    examples.append(
        (
            "John is either happy or sad.",
            ("tell", Xor(Atom("Happy", "john"), Atom("Sad", "john"))),
        )
    )
    examples.append(
        (
            "John lives in Seattle or Portland.",
            (
                "tell",
                Or(
                    Atom("Live_in", "john", "seattle"),
                    Atom("Live_in", "john", "portland"),
                ),
            ),
        )
    )
    examples.append(
        (
            ...
            (
                "tell",
                And(
                    Or(Atom("Rain"), Atom("Snow")), Not(And(Atom("Rain"), Atom("Snow")))
                ),
            ),
        )
    )
    ...
    examples.append(
        (
            "Cats are animals.",
            ("tell", Forall("$x1", Implies(Atom("Cat", "$x1"), Atom("Animal", "$x1")))),
        )
    )
    examples.append(
        (
    ...
    examples.append(
        (
            "If a person lives in California, he is happy.",
            (
                "tell",
                Forall(
                    "$x1",
                    Implies(
                        And(
                            Atom("Person", "$x1"), Atom("Live_in", "$x1", "california")
                        ),
                        Atom("Happy", "$x1"),
                    ),
                ),
    ...
    examples.append(
        (
            "John visited every city.",
            (
                "tell",
                Forall(
                    "$x1", Implies(Atom("City", "$x1"), Atom("Visit", "john", "$x1"))
                ),
            ),
        )
    )
    examples.append(
        (
            "Every city was visited by John.",
            (
                "tell",
                Forall(
                    "$x1", Implies(Atom("City", "$x1"), Atom("Visit_by", "$x1", "john"))
                ),
            ),
        )
    )
            ...
            (
                ...
                Forall(
                    ...
                    Implies(
                        Atom("Person", "$x1"),
                        Exists(
                            "$x2", And(Atom("Cat", "$x2"), Atom("Like", "$x1", "$x2"))
                        ),
                    ),
                ),
            ),
    ...
    examples.append(
        (
            ...
            (
                "tell",
                Not(Exists("$x1", And(Atom("City", "$x1"), Atom("Perfect", "$x1")))),
            ),
        )
    )

    examples.append(
        ("Does John live in Sweden?", ("ask", Atom("Live_in", "john", "sweden")))
    )
    ...
    for iteration in range(0, 10):
        ...
        for x, y in examples:
            ...
            derivations = parse_utterance(utterance, rules)
            targetDeriv = None
            for deriv in derivations:
            ...
            if predDeriv != targetDeriv:
                print(
                    "Mistake: %s => %s, predicted %s"
                    % (x, rstr(y), rstr(predDeriv.form))
                )
                numMistakes += 1
                # Update weights
                numUpdates += 1
