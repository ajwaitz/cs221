...
argparser.add_argument(
    "--inference",
    default="resolution",
    help="Inference algorithm (either resolution or modelChecking)",
)
argparser.add_argument("--utterances", help="Utterances to load before the prompt")
argparser.add_argument(
    "--nltk", help="Use NLTK to analyze sentences", action="store_true", default=False
)
...
class NaturalLanguageInterface:
    def __init__(self):
        ...
        if opts.grammar == "submission":
            self.rules = nlparser.create_base_english_grammar()
            for createRule in [
                submission.create_rule1,
                submission.create_rule2,
                submission.create_rule3,
            ]:
                try:
                    self.rules.append(createRule())
                except:
                    print("Error creating rule...skipping.")
            self.processor = nlparser.create_base_language_processor()
        elif opts.grammar == "standard":
            self.rules = nlparser.create_standard_english_grammar()
            self.processor = (
                nlparser.NLTKProcessor if opts.nltk else nlparser.SimpleProcessor
            )
        else:
            raise Exception("Unknown grammar: either submission or standard")
        if opts.train:
            nlparser.train_grammar(self.rules)
    ...
    def init_kb(self):
        if opts.inference == "resolution":
            self.kb = logic.create_resolution_kb()
        elif opts.inference == "modelChecking":
            self.kb = logic.create_model_checking_kb()
        else:
    ...
    def process(self, line):
        ...
        if self.verbose >= 1:
            print(("Shallow analysis:", utterance))
        ...
        # Semantic parsing
        derivations = nlparser.parse_utterance(
            utterance, self.rules, verbose=self.verbose
        )
        ...
        if mode == "ask":
            response = self.kb.ask(form)
        if mode == "tell":
            response = self.kb.tell(form)
        print()
        logic.show_kb_response(response, self.verbose)
...
NaturalLanguageInterface().interactive_loop()
